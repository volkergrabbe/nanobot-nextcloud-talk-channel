...
class MatrixConfig(Base):
    ...
    device_id: str = ""  # required for E2EE and proper authentication
    allowFrom: list[str] = Field(default_factory=list)  # e.g. ["@alice:example.com"]
    roomPolicy: str = "mention"  # "mention" | "open" | "dm"
    allowRooms: list[str] = Field(
        default_factory=list
    )  # room IDs to whitelist (empty = all)
    e2eeEnabled: bool = True  # enable end-to-end encryption by default
    maxMediaBytes: int = 10485760  # 10MB default media limit (0 = block all)
...
class NextcloudTalkConfig(Base):
    ...
    allow_from: list[str] = Field(
        default_factory=list
    )  # Nextcloud user IDs (empty = all)
    allowRooms: list[str] = Field(
        default_factory=list
    )  # Conversation tokens (empty = all)
